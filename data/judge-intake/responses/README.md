...
Save the judge model's raw combined response here as `judge-response-<version>.json` (e.g. `judge-response-v1.json`) — the single JSON object, keyed by task ID, that came back after uploading `docs/judge-run-<version>.md`.
