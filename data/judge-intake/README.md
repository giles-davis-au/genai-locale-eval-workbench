...
They're not hand-written: upload `docs/judge-run-<version>.md` to the external judge, save its single combined response as `docs/judge-response-<version>.json`, then run `python3 scripts/split_judge_response.py <version> docs/judge-response-<version>.json` to explode it into the individual files here.
