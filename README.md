...
3. **Open the [live demo](https://giles-davis-au.github.io/genai-locale-eval-workbench/)** — no cloning or local setup needed (see "How can it be run safely?" below if you'd rather run it locally) — then go straight to **View 3** (V1 dashboard) rather than View 1 or 2: fastest way to see the tool compute something real, and it surfaces the human-vs-judge disagreements that motivate everything downstream.
