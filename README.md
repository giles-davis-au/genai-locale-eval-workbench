...
**Phase 3 of 5 complete, Phase 4 underway.** All 10 V1 tasks have a frozen raw output, a reviewed and approved human evaluator assessment, and an imported LLM-as-a-judge assessment (`data/v1-results.json`); see [docs/provenance.md](docs/provenance.md) for the human-evaluator-vs-LLM-as-a-judge disagreement rate. `data/findings.json` holds three evidence-linked findings from that V1 evidence (see [docs/methodology.md](docs/methodology.md)): two propose a system change, both now bundled into the same V2 revision rather than tested as separate versions (see [docs/limitations.md](docs/limitations.md) for that trade-off), and one proposes evaluator training, not yet actioned. `data/monitoring-notes.json` holds one further piece of real signal that stops short of a finding (a single-instance word-count violation, disclosed rather than silently dropped). `data/v2-results.json` now holds all 10 V2 records too: deterministically retrieved context, a freshly generated V2 output for every task, and a draft human evaluator assessment for each, all still `review_status: "pending_review"` and **not yet approved by Giles**; the LLM-as-a-judge run for V2 hasn't happened yet either. The app has five views: **View 1** (setup), **View 2** (V1 evaluation: inspect any one of the 10 records), **View 3** (V1 dashboard: calculated summaries, filters, drilldown) and **View 4** (investigate the pattern and recommendation) are implemented; **View 5** (compare V1 vs V2) is not yet, though `data/v2-results.json` can already be inspected directly. Do not treat anything in this repository as a finished evaluation until this section says otherwise. See [Implementation phases](#implementation-phases) below for what each subsequent phase adds.
...
4. **V2 context and paired comparison** *(retrieval and generation done, review and paired comparison view not yet)*: deterministic glossary retrieval (`scripts/build_v2_context.py`) and all 10 V2 outputs generated, with a draft human evaluator assessment awaiting Giles's review; the LLM-as-a-judge run and the paired comparison view (View 5) are still to come.
...
Checks structural integrity: exactly ten unique task IDs, one V1 and one V2 result per task, valid rubric references, recomputed points/status matching stored values, findings referencing real records, required provenance fields present, and no duplicate IDs. At this phase it correctly reports zero errors and 20 warnings: one per V2 record for the still-`pending_review` human evaluator assessment, and one per V2 record for the LLM-as-a-judge run still `awaiting_external_run`.
