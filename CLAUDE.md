...
3. `data/locale-profiles.json` + `data/terminology.csv`: the material V2's context draws from. Every task receives the same locale profile and the entire terminology glossary, unfiltered (see [docs/data-schema.md](docs/data-schema.md) for the exact rule); no per-task matching, embeddings, or external retrieval service.
...
| `scripts/build_v2_context.py` | Computes each task's deterministic V2 `retrieved_context` (locale profile + the full terminology glossary, supplied unfiltered) from `data/tasks.json`, `data/terminology.csv` and `data/locale-profiles.json`. Does not generate marketing copy; that's the model under test's own work, hand-assembled into `data/v2-results.json` alongside this script's output. |
...
See [docs/limitations.md](docs/limitations.md) for the current, complete list of what this repository does and doesn't demonstrate: it's the canonical source, and this file doesn't restate it.

## Current build status

All 5 phases complete, including Phase 5 (hardening and handoff: completed documentation, full validation, offline/no-network confirmation). A 2026-09-11 amendment redesigned `data/terminology.csv` and regenerated, re-reviewed and re-judged T09 and T10's V2 records as a result (see [docs/provenance.md](docs/provenance.md)); all 20 V1+V2 records are fully approved/imported and `scripts/validate_data.py` reports 0 errors, 0 warnings. See the "Project status" section at the top of [README.md](README.md) for the full current-state summary. If this repository is extended further (a new task, a new locale, a new finding), update that section, this file, and `docs/` together; do not let them drift out of sync with the actual data files.
