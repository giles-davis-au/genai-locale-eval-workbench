...
- **A real, disclosed instance of judge/human divergence (Finding 2 in `data/findings.json`) recommends evaluator training, not a system change, and neither happens within this build.** The judge flags `accuracy`/`addition` far more often than the human evaluator did, and on inspection most of the LLM's flags are legitimate: this project treats it as a human-evaluator calibration gap rather than a problem with the V1 generation system configuration. That is a scoping choice for a ten-task, single-reviewer demonstration, not a claim that the underlying calibration gap should go unaddressed in a real evaluation system. See [methodology.md](methodology.md) for the alternative-explanations check behind that call and the optional, non-actioned recommendations a production system might consider instead.
