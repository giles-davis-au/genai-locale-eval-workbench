...
- **A real, disclosed instance of judge/human divergence on `accuracy`/`addition` split into two findings, and neither recommendation has been actioned within this build.** The judge flags `accuracy`/`addition` far more often than the human evaluator did, and on inspection most of the LLM's flags are legitimate. Finding 2 (in `data/findings.json`) treats this as a genuine V1 generation defect and recommends a system-instruction change; Finding 3 treats the human evaluator's original miss rate on the same defect as a separate, evaluation-side calibration gap and recommends evaluator training instead. Neither recommendation has been built or actioned yet, a scoping choice for a ten-task, single-reviewer demonstration, not a claim that either gap should go unaddressed in a real evaluation system. See [methodology.md](methodology.md) for the alternative-explanations check behind both findings and the optional, non-actioned recommendations a production system might consider instead.
