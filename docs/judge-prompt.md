# LLM-as-a-judge prompt and intake process

This project's provisional LLM-as-a-judge assessments are **not** produced by Claude. They are produced independently by Giles, uploading a generated file (`docs/judge-run-<version>.md`, see below) to a separate model (GPT-5.6 Sol), then importing its response into this repository. This document explains that process and is the reusable source for the generated file, so the process is fully inspectable.

## Why a separate model, and why blind

The human evaluator assessment, the app, and this documentation were all produced with help from the same assistant (Claude) in the same session. Using that same assistant as the "independent" LLM-as-a-judge would be a real conflict of interest, not just an appearance of one. Using a different model (GPT-5.6 Sol), run by Giles outside this session, keeps the LLM-as-a-judge genuinely independent of the human-evaluator-assessment drafting process.
...
4. Split it into the per-record files the importer expects:
   ...
   ```bash
   python3 scripts/split_judge_response.py v1 data/judge-intake/responses/judge-response-v1.json
   ```
...
Until a given task's LLM-as-a-judge file is imported, that record's `judge_assessment.import_status` stays `"awaiting_external_run"` and the UI shows it as pending rather than displaying placeholder data.

## Partial re-runs (e.g. `docs/judge-run-v2.1.md`)

The normal process above batches all 10 tasks of a version into one upload. When only a subset of a version's output text actually changes after the fact (see [provenance.md](provenance.md)'s 2026-09-11 terminology-glossary redesign, which regenerated only T09 and T10's V2 output), re-judging all 10 again would add cost without adding signal: the other 8 records' output is byte-identical to what was already judged. In that case a scoped file covering just the affected task IDs is generated by hand, following this document's exact format (role, rubric, output-format spec, then only the affected tasks), named `docs/judge-run-<version>.<n>.md` to distinguish it from the full per-version file. The import steps are unchanged: `scripts/split_judge_response.py` and `scripts/import_judge_results.py` both key off `task_id`/version pairs, not off which file they came from, so a partial response splits and imports exactly like a full one, overwriting only the record(s) actually present in it.
